...
("크리에이터 시딩","마이크로 (UGC)","7~12월","IG·TikTok",
 ...
 "직장인 루틴형 2535 — 웰니스 루틴 보유·편의성 중시","#올더베러 #오늘도베러 #데일리웰니스",235,"마이크로",500000,False),
("크리에이터 시딩","나노","7~12월","IG",
 "다수 진성 후기로 ‘익숙함·신뢰’ 형성 · 검색·해시태그 자산 축적(#오늘도베러)",
 "웰니스 입문형 2030 — 선택 장벽·가격 민감","#올더베러 #오늘도베러 #웰니스입문",235,"나노",250000,False),
...
C(ms,18,1,"월 시딩 총건수",b=True,fill=SUB,al="left")
for i in range(6):
    cl=get_column_letter(3+i); C(ms,18,3+i,f"=SUM({cl}6:{cl}11)",b=True,fill=SUB)
C(ms,18,2,"",fill=SUB); C(ms,18,9,"=SUM(C18:H18)",b=True,fill=SUB); C(ms,18,10,"",fill=SUB)
C(ms,19,9,"총 견적",b=True,al="right"); C(ms,19,10,"=SUM(J6:J16)",fmt=ACC,b=True,fill=GREEN_HL)
for r in range(5,17): ms.row_dimensions[r].height=19
